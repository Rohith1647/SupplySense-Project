...
## 🤖 Agent Prompt — Update Paper With Real Benchmark Results

> **Use this section once you have run `benchmark/run_real_benchmark.py` on the RTX 4050 machine and have the real output numbers.**
>
> Clone this repo, open it as your workspace in Antigravity, then paste the following prompt with your actual results filled in:

---

```
I have run the real SupplySense benchmark (benchmark/run_real_benchmark.py)
on the RTX 4050 machine using actual Ollama LLM inference and got the
following terminal output:

--- PASTE YOUR FULL TERMINAL OUTPUT HERE ---

Also attaching the contents of benchmark/results_by_category.csv:

--- PASTE CSV CONTENTS HERE ---

Please do all of the following:
1. Update TABLE I in PAPER_COMPLETE_DRAFT.md with the real Llama-3 and Mistral
   F1, Precision, Recall, and Mean Latency values from the output above.
2. Update TABLE III in PAPER_COMPLETE_DRAFT.md with the real KG traversal
   latency (ms) and real BOM coverage (%) printed in the benchmark output.
3. Update the Abstract and Section X Conclusion in PAPER_COMPLETE_DRAFT.md
   to reflect the real F1 score (replace placeholder values if they changed).
4. Regenerate Figure 4 (fig4_precision_recall_f1.png) with the real numbers
   by running the plotting section with updated values.
5. Commit and push all updated files to the GitHub repo.
6. Tell me if the real results are strong enough for journal submission and
   which specific journal to target first based on the results.
```

---

> **Have these ready before giving the prompt:**
> - Full terminal output from `python run_real_benchmark.py` (copy-paste everything)
> - Contents of `benchmark/results_by_category.csv` (generated by the script)
> - This repo cloned and open as the workspace in Antigravity
